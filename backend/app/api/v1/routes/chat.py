...
async def _handle_image_gen(message: str, language: str) -> str:
    ...
    if not result.get("success"):
        ...
        if status == "downloading":
            return (
                f"⏳ {err}" if language != "bn" else
                f"⏳ প্রথমবার ছবি বানানোর জন্য দরকারি model background-এ download হচ্ছে। "
                f"{err}\n\n(ইন্টারনেট স্পিড অনুযায়ী কিছুক্ষণ সময় লাগতে পারে — একটু পরে আবার চেষ্টা করো।)"
            )
        if status in ("insufficient_disk", "insufficient_ram"):
            return (
                f"⚠️ {err}" if language != "bn" else
                f"⚠️ Model download করার মতো জায়গা/RAM এখন যথেষ্ট নেই।\n{err}"
            )
        return (
...
async def _handle_transform_image(message: str, staged: dict | None, language: str) -> str:
    ...
    if not result.get("success"):
        ...
        if status == "downloading":
            return (
                f"⏳ প্রথমবার transform করার জন্য দরকারি model background-এ download হচ্ছে। "
                f"{err}\n\n(একটু পরে আবার চেষ্টা করো।)" if language == "bn"
                else f"⏳ {err}"
            )
        if status in ("insufficient_disk", "insufficient_ram"):
            return (
